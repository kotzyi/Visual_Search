...
def main():
    ...
    #condition의 역할은 현재는 각 도메인의 수라고 생각하면 됨.ex dress-bag:0 dress-bag:1
    global conditions
    ...
    train_loader = torch.utils.data.DataLoader(
        TripletImageLoader('data', 'shoe-dress-images', 'filenames_bag.json', 
            conditions, 'train', n_triplets=args.num_traintriplets*4,
                        transform=transforms.Compose([
                            transforms.Scale(200), #바꾸면 에러
    ...
    test_loader = torch.utils.data.DataLoader(
        TripletImageLoader('data', 'shoe-dress-images', 'filenames_bag.json', 
            conditions, 'test', n_triplets=20000*4,
                        transform=transforms.Compose([
    ...
    val_loader = torch.utils.data.DataLoader(
        TripletImageLoader('data', 'shoe-dress-images', 'filenames_bag.json', 
            conditions, 'val', n_triplets=20000*4,
                        transform=transforms.Compose([
                            transforms.Scale(200),
                            transforms.CenterCrop(200),
                            transforms.ToTensor(),
    ...
    #csn 모델의 역할: resnet을 embedding하고 있으며, resnet으로 부터 feature vector를 받아 mask를 학습시킨다. 
    ...
    #triplet network의 역할: csn을 embedding하고 있으며, 입력 받은 이미지 vector로 부터 mask norm을 계산하고 loss를 계산하여 학습시킨다.
